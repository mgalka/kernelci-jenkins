...
pipelineJob('lava-bisection') {
  ...
  parameters {
    ...
    stringParam('LAVA_CALLBACK', 'kernel-ci-callback-staging', 'Description of the LAVA auth token to look up and use in LAVA callbacks')
    stringParam('EMAIL_RECIPIENTS', 'gtucker@collabora.com', 'List of recipients for all emails generated by this job')
    stringParam('LABS_WHITELIST', '', 'If defined, jobs will abort if the LAB is not on that list.')
    stringParam('TREES_WHITELIST', '', 'If defined, jobs will abort if the KERNEL_TREE is not on that list.')
